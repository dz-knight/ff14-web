...
- Hardened the local preview server against encoded path traversal, repository-file exposure, unsupported methods, symlink escape, and oversized icon downloads
- Added deterministic regression tests for slow responses, refresh races, focus recovery, network timeout/cancellation, search races, cache recovery, and static-server boundaries
...
- Kept the desktop Wiki fallback reachable when CafeMaker fails, restricted rendered icon and Wiki URLs to trusted formats, and bounded search and response text sizes
- Changed icon proxy downloads to stop reading at 5 MB instead of buffering an oversized response first
...
- Removed the unused legacy `static-overrides.js` runtime and made item-mapping generation use 32-bit icon IDs plus atomic output replacement
